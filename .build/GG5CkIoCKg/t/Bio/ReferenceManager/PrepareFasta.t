...
ok($obj = Bio::ReferenceManager::PrepareFasta->new(fasta_file => 't/data/PrepareFasta/invalid_file.fa', 
reference_store_dir => $tmp_dirname ), 'invalid file init');
...
compare_ok('t/data/PrepareFasta/expected_odd_chars_in_sequence.fa',
    $obj->final_output_filename(),
    'fix odd characters in sequence'
);
ok(-e $tmp_dirname.'/dos2unix.fa', 'dos2unix temp file exists');
ok(-e $tmp_dirname.'/acgtn_only.fa', 'acgtn_only.fa temp file exists');
ok(-e $tmp_dirname.'/sort_by_name.fa', 'sort_by_name.fa temp file exists');
